...
class NewsfeedServiceTest(TestCase):
    ...
    def setUp(self):
        super(NewsfeedServiceTest, self).setUp()
    ...
    def test_get_user_newsfeeds(self):
        newsfeed_timestamps = []
        for i in range(3):
            tweet = self.create_tweet(self.fiona)
            newsfeed = self.create_newsfeed(self.marcus, tweet)
            newsfeed_timestamps.append(newsfeed.created_at)
        newsfeed_timestamps = newsfeed_timestamps[::-1]

        # cache miss
        newsfeeds = NewsFeedService.get_cached_newsfeeds(self.marcus.id)
        ...
        new_newsfeed = self.create_newsfeed(self.marcus, tweet)
        newsfeeds = NewsFeedService.get_cached_newsfeeds(self.marcus.id)
        newsfeed_timestamps.insert(0, new_newsfeed.created_at)
        self.assertEqual([f.created_at for f in newsfeeds], newsfeed_timestamps)

    def test_create_new_newsfeed_before_get_cached_newsfeeds(self):
        feed1 = self.create_newsfeed(self.marcus, self.create_tweet(self.marcus))

        self.clear_cache()
        conn = RedisClient.get_connection()

        key = USER_NEWSFEEDS_PATTERN.format(user_id=self.marcus.id)
        self.assertEqual(conn.exists(key), False)
        feed2 = self.create_newsfeed(self.marcus, self.create_tweet(self.marcus))
        self.assertEqual(conn.exists(key), True)

        newsfeeds = NewsFeedService.get_cached_newsfeeds(self.marcus.id)
        self.assertEqual([f.created_at for f in newsfeeds], [feed2.created_at, feed1.created_at])
...
class NewsFeedTaskTests(TestCase):
    ...
    def test_fanout_main_task(self):
        ...
        msg = fanout_newsfeeds_main_task(tweet.id, created_at, self.marcus.id)
        self.assertEqual(msg, '1 newsfeeds going to fanout, 1 batches created.')
        self.assertEqual(1 + 1, NewsFeedService.count())
        cached_list = NewsFeedService.get_cached_newsfeeds(self.marcus.id)
        self.assertEqual(len(cached_list), 1)

        for i in range(2):
            user = self.create_user('user{}'.format(i))
            self.create_friendship(user, self.marcus)
        tweet = self.create_tweet(self.marcus, 'tweet 2')
        if GateKeeper.is_switch_on('switch_newsfeed_to_hbase'):
            created_at = tweet.timestamp
        else:
            created_at = tweet.created_at
        ...
        self.create_friendship(user, self.marcus)
        tweet = self.create_tweet(self.marcus, 'tweet 3')
        if GateKeeper.is_switch_on('switch_newsfeed_to_hbase'):
            created_at = tweet.timestamp
        else:
            created_at = tweet.created_at
        ...
        cached_list = NewsFeedService.get_cached_newsfeeds(self.fiona.id)
        self.assertEqual(len(cached_list), 3)